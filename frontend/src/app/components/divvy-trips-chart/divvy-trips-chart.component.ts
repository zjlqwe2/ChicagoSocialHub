...
export class DivvyTripsChartComponent implements OnInit {
  ...
  constructor(private placesService: PlacesService, private router: Router, private http: HttpClient,fb: UntypedFormBuilder) {
    console.log("DivvyTripsChartComponent constructor()")
  ...
  ngOnInit() {
      // Default display is for first week of 2018 fourth quarter  //默认显示时间为2018年第四季度第一周
      console.log("DivvyTripsChartComponent ngOnInit()")
      this.minDate = new Date('10/01/2018');
      this.maxDate = new Date('12/31/2018');
      //这个会自动 生成一个  data array
      this.dates_of_week_days = this.getDates('2018-10-01','2018-10-20');
      console.log("dates_of_week_days  ",this.dates_of_week_days);  //时间数组
      for (var i = 0; i < this.dates_of_week_days.length; i++) {
        var daydata = new Date(this.dates_of_week_days[i]).getDay()+1
        console.log("waht is this time",daydata ,typeof(daydata));  //时间数组
  ...
  getDivvyTripsCounts(day) {
        if(day == 0){
          this.chart_7_name = 'Sunday';
           this.placesService.getDivvyTripsCountsForSunday().subscribe((data: divvyTripsCountPerDay[]) => {
          ...
          this.placesService.getDivvyTripsCountsForMonday().subscribe((data: divvyTripsCountPerDay[]) => {
              this.divvyTripsCounts = data;
          ...
          this.placesService.getDivvyTripsCountsForTuesday().subscribe((data: divvyTripsCountPerDay[]) => {
              this.divvyTripsCounts = data;
          ...
          this.placesService.getDivvyTripsCountsForWednesday().subscribe((data: divvyTripsCountPerDay[]) => {
            this.divvyTripsCounts = data;
            var id="#svg3"; this.initSvg(id);
            this.initAxis();
            this.drawAxis();
            this.updateChartBars(this.divvyTripsCounts);})
          console.log("into Wednesday")
        }else if(day == 4) {
          this.chart_4_name = 'Thursday';
          this.placesService.getDivvyTripsCountsForThursday().subscribe((data: divvyTripsCountPerDay[]) => {
            this.divvyTripsCounts = data;
            var id="#svg4"; this.initSvg(id);
            this.initAxis();
            this.drawAxis();
            this.updateChartBars(this.divvyTripsCounts);})
          console.log("into Thursday")
        }else if(day == 5) {
          this.chart_5_name = 'Friday';
          this.placesService.getDivvyTripsCountsForFriday().subscribe((data: divvyTripsCountPerDay[]) => {
            this.divvyTripsCounts = data;
            var id="#svg5"; this.initSvg(id);
            this.initAxis();
            this.drawAxis();
            this.updateChartBars(this.divvyTripsCounts);})
          console.log("into Friday ")
        }else if(day == 6) {
          this.chart_6_name = 'Saturday';
          this.placesService.getDivvyTripsCountsForSaturday().subscribe((data: divvyTripsCountPerDay[]) => {
            this.divvyTripsCounts = data;
            var id="#svg6"; this.initSvg(id);
            this.initAxis();
            this.drawAxis();
            this.updateChartBars(this.divvyTripsCounts);})
          console.log("into Saturday ")
        }
