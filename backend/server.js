...
var conString = "pg://postgres:123456@127.0.0.1:5432/chicago_divvy_stations_status";
var pg_connection_divvy_trips = "pg://postgres:123456@127.0.0.1:5432/chicago_divvy_trips";

// var conString = "tcp://postgres:123456@localhost/nodejspg"; //tcp://用户名：密码@localhost/数据库名
...
//stations_found all the review info
router.route('/stations/getdocks').post((req, res) => {

    var str = JSON.stringify(req.body, null, 8);

    for (var i = 0,len = stations_found.length; i < len; i++) {

        if ( stations_found[i].stationName === req.body.placeName ) { // strict equality test // 严格相等测试
...
router.route('/stations/find').post((req, res) => {
  console.log("进入find方法  22")
    var str = JSON.stringify(req.body, null, 4);  //只取其中4條數據
    console.log("str"+str)
    console.log("req.body.placeName"+req.body.placeName)
    for (var i = 0,len = places_found.length; i < len; i++) {
        if ( places_found[i].name === req.body.placeName ) { // strict equality test 严格相等测试
            place_selected = places_found[i];
            break;
        }
    }
    console.log("筛选后的"+place_selected)
